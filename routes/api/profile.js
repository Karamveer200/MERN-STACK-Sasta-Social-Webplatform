...
// @route   POSTapi/profile/projURL
// @desc    Add github proj URL
// @access  Private
router.put("/projurl", auth, async (req, res) => {
  try {
    const { url } = req.body;
    console.log("urlllll", url);
    console.log("bodyy- ", req.body);
    const profile = await Profile.findOne({ user: req.user.id });
    profile.url.unshift(url);
  ...
  try {
    const options = {
      uri:
        ...
        req.params.username +
        "/repos?per_page=10&sort=created:asc&client_id=" +
